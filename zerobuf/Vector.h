...
public:
    ...
    public:
        ...
        template <class Q = U, typename = typename std::enable_if<
                                   !std::is_const<Q>::value>::type>
    ...
    const T* data() const
    {
        return const_cast<const Allocator*>(_alloc)->template getDynamic<T>(
            _index);
    }
    ...
    template <class Q = T>
    void push_back(const typename std::enable_if<
    ...
    template <class Q = T>
    void fromJSON(const Json::Value& json,
                  const typename std::enable_if<std::is_enum<Q>::value,
                                                Q>::type* = nullptr);

    /** Update this vector of arithmetics from its JSON representation. */
    template <class Q = T>
    void fromJSON(const Json::Value& json,
                  const typename std::enable_if<std::is_arithmetic<Q>::value,
                                                Q>::type* = nullptr);
    ...
    void fromJSON(
        ...
        const typename std::enable_if<std::is_same<Q, servus::uint128_t>::value,
                                      Q>::type* = nullptr);
    ...
    void toJSON(
        Json::Value& json,
        const typename std::enable_if<std::is_base_of<Zerobuf, Q>::value,
                                      Q>::type* = nullptr) const;
    ...
    template <class Q = T>
    void fromJSONBinary(const Json::Value& json,
                        const typename std::enable_if<std::is_pod<Q>::value,
                                                      Q>::type* = nullptr);

    /** @return the JSON representation of this vector, with base64 encoding. */
    template <class Q = T>
    void toJSONBinary(Json::Value& json,
                      const typename std::enable_if<std::is_pod<Q>::value,
                                                    Q>::type* = nullptr) const;
...
// Implementation
template <class T>
inline Vector<T>::Vector(Allocator& alloc, const size_t index)
    : _alloc(&alloc)
    , _index(index)
{
}
...
template <class T>
inline void Vector<T>::resize(const size_t size_)
{
...
template <class T>
template <class Q>
inline const typename std::enable_if<!std::is_base_of<Zerobuf, Q>::value,
                                     Q>::type& Vector<T>::
    operator[](const size_t index) const
{
    if (index >= size())
...
template <class T>
template <class Q>
inline typename std::enable_if<!std::is_base_of<Zerobuf, Q>::value, Q>::type&
...
template <class T>
template <class Q>
inline void Vector<T>::push_back(
    const typename std::enable_if<!std::is_base_of<Zerobuf, Q>::value, Q>::type&
        value)
{
    const size_t size_ = _getSize();
        ...
        value)
{
    ...
    uint8_t* newPtr =
        _alloc->updateAllocation(_index, true /*copy*/, size_ + zerobuf.size);
    ::memcpy(newPtr + size_, zerobuf.ptr.get(), zerobuf.size);
}

template <class T>
template <class Q>
inline void Vector<T>::fromJSON(
    const Json::Value& json,
    const typename std::enable_if<std::is_base_of<Zerobuf, Q>::value, Q>::type*)
{
    ...
    const typename std::enable_if<std::is_enum<Q>::value, Q>::type*)
{
    ...
    for (size_t i = 0; i < size_; ++i)
        array[i] = string_to_enum<T>(
            zerobuf::fromJSON<std::string>(getJSONField(json, i)));
}
...
template <class T>
template <class Q>
inline void Vector<T>::toJSON(
    Json::Value& json,
    const typename std::enable_if<std::is_base_of<Zerobuf, Q>::value, Q>::type*)
    const
{
    const size_t size_ = size();
    zerobuf::emptyJSONArray(
        json); // return [] instead of null if array is empty
    for (size_t i = 0; i < size_; ++i)
...
inline void Vector<T>::toJSON(
    Json::Value& json,
    const typename std::enable_if<std::is_enum<Q>::value, Q>::type*) const
{
    const size_t size_ = size();
    zerobuf::emptyJSONArray(
        json); // return [] instead of null if array is empty
    for (size_t i = 0; i < size_; ++i)
        zerobuf::toJSON(enum_to_string<T>((*this)[i]), getJSONField(json, i));
}
...
inline void Vector<T>::toJSON(
    Json::Value& json,
    const typename std::enable_if<std::is_arithmetic<Q>::value, Q>::type*) const
{
    const size_t size_ = size();
    zerobuf::emptyJSONArray(
        json); // return [] instead of null if array is empty
...
template <class T>
template <class Q>
inline void Vector<T>::fromJSONBinary(
    const Json::Value& json,
...
template <class T>
template <class Q>
inline void Vector<T>::toJSONBinary(
    Json::Value& json,
...
template <class T>
inline void Vector<T>::copyBuffer(uint8_t* data_, size_t size_)
{
...
template <>
inline std::ostream& operator<<(std::ostream& os, const Vector<char>& string)
{
