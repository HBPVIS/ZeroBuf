...
{
template <class A>
DynamicSubAllocatorBase<A>::DynamicSubAllocatorBase(A& parent,
...
template <class A>
uint8_t* DynamicSubAllocatorBase<A>::getData()
{
    return _parent.template getDynamic<uint8_t>(_header) + _element * _size;
}

template <>
uint8_t* DynamicSubAllocatorBase<const Allocator>::getData()
{
    throw std::runtime_error("Non-const data access on const data");
}

template <class A>
const uint8_t* DynamicSubAllocatorBase<A>::getData() const
{
...
template <class A>
void DynamicSubAllocatorBase<A>::copyBuffer(const void* data, const size_t size)
{
    if (size != _size)
