...
template <class A>
uint8_t* StaticSubAllocatorBase<A>::getData()
{
    return _parent.getData() + _offset;
}

template <>
uint8_t* StaticSubAllocatorBase<const Allocator>::getData()
{
...
template <class A>
void StaticSubAllocatorBase<A>::copyBuffer(const void* data, const size_t size)
{
    if (size != _size)
