...
#define TESTVALUES(type, name)                                                 \
    const std::vector<type> expected##name##Vector{type(1), type(1), type(2),  \
                                                   type(3)};                   \
    ...
    const type& name##Value(object.get##name##value());                        \
                                                                               \
    BOOST_CHECK_EQUAL(expected##name##Vector.size(), name##Dynamic.size());    \
    BOOST_CHECK_EQUAL(expected##name##Vector.size(), name##Array.size());      \
...
void checkTestObject(const test::TestSchema& object)
{
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(testEnumDynamic.begin(),
                                  testEnumDynamic.end(), result2.begin(),
                                  result2.end());
