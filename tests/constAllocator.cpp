...
BOOST_AUTO_TEST_CASE(construction)
{
    ...
    BOOST_CHECK_THROW(mutableBaseAlloc->copyBuffer(nullptr, 0),
                      std::runtime_error);
    BOOST_CHECK_THROW(mutableBaseAlloc->updateAllocation(0, false, 0),
                      std::runtime_error);
...
BOOST_AUTO_TEST_CASE(create)
{
    ...
    test::ConstTestSchemaPtr newObject =
        test::TestSchema::create(data.ptr.get(), data.size);
